...
#
"""
Global controls, a channel rack.

Patterns are persistent per synth. They are queried via abs time, and
give up notes for the time window. Each editor is passed a pattern
which is shared by the engine. Select new sample, pass new pattern,
update editor.

Patterns play the sample once per period, or contain random
notes. They are saved and dragged from the selector onto the synth
where they can be editted. This requires a solid visual concept of the
synth's.
"""
...
class EditorApp(QFrame):
    ...
    def slotVolume(self, index, value):
	""" """
#        self.engine.server.sendMsg('/n_set', 
