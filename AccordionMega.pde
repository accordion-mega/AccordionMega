...
boolean DEBUG = true;
boolean DUMB = true;
boolean DUMB_LEFT = false;
boolean DUMB_RIGHT = false;
...
//  that's 8 bits of PortF, 8 bits of PortK and they are read as a byte and byte later.
...
byte register_pins_right[] = {43,42,41,40};
byte register_pins_left[] =  {39,38,37};
bool use_left_register = 1;
bool use_right_register = 1;
byte midi_channel_num_right[] = {36,35};
byte dumb_switch_left = 34;
byte dumb_switch_right = 33;
...
const prog_char *register_names_left[] = {
  st_regL_1,st_regL_2,st_regL_3,st_regL_4,st_regL_5,st_regL_6,st_regL_7
};
...
void setup() {  
  ...
  init_key_pins(left_row_pins,sizeof(left_row_pins));

  // initializing pins which define if hand uses pressure
  init_dumb_pins();
  am_i_dumb();
...
void loop() {
  ...
  redefine_register(register_pins_right, sizeof(register_pins_right), false);
  redefine_register(register_pins_left, sizeof(register_pins_left), true);
  //Serial.print(curr_register_left,DEC);
  //Serial.print("  ");
  //Serial.println(curr_register_right,DEC); 
...
void am_i_dumb(){
  if(!digitalRead(dumb_switch_left) && DUMB){
    DUMB=false;
    if (DEBUG){
      Serial.println("Going smart");
    }
    recalibrate();
  }
  if (digitalRead(dumb_switch_left) && !DUMB){ 
